...
export default class RichTextArea extends Component {
  ...
  proxyEvents(props, nextProps) {
    ...
    for (handler in nextHandlers)
      if (nextHandlers.hasOwnProperty(handler)) {
    ...
    for (handler in currHandlers)
      if (currHandlers.hasOwnProperty(handler)) {
  ...
  render({ value, className, placeholder, stylesheet, ...props }) {
    ...
    return (
      <div {...props} onFocus={this.doFocus} tabIndex=" " is-placeholder={(!value && !!placeholder) || null}>
        <iframe />
      </div>
    )
  }
