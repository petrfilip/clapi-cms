    ...
    return (
        <TypeDefinitionDropArea
            ...
            onDropEvent={(componentKey) => {
                setActionSidebar(<TypeDefinitionInputSettings values={{ position: index }} onConfirm={onNewDefinition} componentKey={componentKey} />)
            }}
        />
    )
}
    ...
    return (
        <ActionButtons>
            ...
            <ActionButton
                onClick={() =>
                    setActionSidebar(
                        <TypeDefinitionInputSettings values={{ position: index, fieldName: item.config.label, apiKey: key }} position={index} onConfirm={onNewDefinition} componentKey={item.type} />
                    )
                }
            >
                S
...
function getComponentPlaceholder(key, item, index, onRemoveDefinition, setActionSidebar, onNewDefinition, onUpdateDefinition) {
    return (
...
function getTypeDefinitionBuilderContainer(typeDefinitionConfig, setActionSidebar, onNewDefinition, onRemoveDefinition, onUpdateDefinition) {
    return (
        <>
            {Object.keys(typeDefinitionConfig).map((item, index) => {
                return (
                    <>
                        ...
                        {getComponentPlaceholder(item, typeDefinitionConfig[item], index, onRemoveDefinition, setActionSidebar, onNewDefinition, onUpdateDefinition)}
                    </>
                )
            })}
...
const TypeDefinitionBuilder = ({ typeDefinitionConfig, onNewDefinition, onRemoveDefinition, onUpdateDefinition }) => {
    ...
    return (
        <TypeDefinitionBuilderContainer>
            {getTypeDefinitionBuilderContainer(typeDefinitionConfig, setActionSidebar, onNewDefinition, onRemoveDefinition, onUpdateDefinition)}
        </TypeDefinitionBuilderContainer>
    )
}
