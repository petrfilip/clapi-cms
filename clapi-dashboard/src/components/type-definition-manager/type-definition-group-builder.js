...
function getDropContainer(typeDefinitionConfig, index, setActionSidebar, onUpdateConfig) {
    return (
        <TypeDefinitionDropArea
            dataTransferKey={'componentKey'}
            onDropEvent={(componentKey) => {
                setActionSidebar(
                    <TypeDefinitionInputSettings values={{ position: index }} onConfirm={(obj) => onNewDefinition(typeDefinitionConfig, obj, onUpdateConfig)} componentKey={componentKey} />
                )
            }}
        />
    )
...
function getComponentPlaceholder(typeDefinitionConfig, key, item, index, setActionSidebar, onUpdateDefinition) {
    return (
        <ComponentPlaceholder>
            ...
            {item.type === 'Group' && getTypeDefinitionBuilderContainer(item.config.fields || {}, setActionSidebar, onUpdateDefinition)}
            <ActionButtons>
...
function getTypeDefinitionBuilderContainer(typeDefinitionConfig, setActionSidebar, onUpdateDefinition) {
    return (
        <>
            {Object.keys(typeDefinitionConfig).map((item, index) => {
                return (
                    <>
                        {getDropContainer(typeDefinitionConfig, index, setActionSidebar, onUpdateDefinition)}
                        {getComponentPlaceholder(typeDefinitionConfig, item, typeDefinitionConfig[item], index, setActionSidebar, onUpdateDefinition)}
                    </>
                )
            })}
            {getDropContainer(typeDefinitionConfig, Object.keys(typeDefinitionConfig).length, setActionSidebar, onUpdateDefinition)}
        </>
    )
}

const onNewDefinition = (config, newDefObj, onUpdateDefinition) => {
    const updatedConfig = addToObject(config, newDefObj.apiKey, newDefObj.value, newDefObj.position)
...
const TypeDefinitionGroupBuilder = ({ typeDefinitionConfig, onUpdateDefinition }) => {
    ...
    return <TypeDefinitionGroupBuilderContainer>{getTypeDefinitionBuilderContainer(typeDefinitionConfig, setActionSidebar, onUpdateDefinition)}</TypeDefinitionGroupBuilderContainer>
}
